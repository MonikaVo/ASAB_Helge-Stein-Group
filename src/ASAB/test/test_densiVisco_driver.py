...
def test_measure():
    ...
    assert Path(f'{conf["densiViscoDriver"]["inputFolder"]}\\{sampleName}.lims').is_file(), f'The file does not exist in the expected directory.'

    # assemble the content as it is expected
    content_target = f"#V100\n1;{sampleName};;;;{method}"
    # get the actual content of the file
    with open(f'{conf["densiViscoDriver"]["inputFolder"]}\\{sampleName}.lims', 'r') as file:
...
def test_check():
    ...
    method = 'Density_02'
    # write the file, which would trigger a measurement if placed in the correct folder - uses the dummy folder in the test configuration
    densiVisco_driver.check(sampleName=sampleName, checktype=check, method=method)

    # check, if the file is generated
    assert Path(f'{conf["densiViscoDriver"]["inputFolder"]}\\{check}.lims').is_file(), f'The file does not exist in the expected directory.'

    # assemble the content as it is expected
    content_target = f"#V200\n1;{sampleName};;;;{method};Type;C (Check);Check Type;{check}"
    # get the actual content of the file
    with open(f'{conf["densiViscoDriver"]["inputFolder"]}\\{check}.lims', 'r') as file:
        content_result = file.read()
    # check, that the actual content matches the expected content
    assert content_result == content_target, f'The content is {content_result} instead of {content_target}.'
...
def test_retrieveData():
    ...
    for f in glob(f'{conf["test_densiViscoDriver"]["savePath"]}\\*.json'):
        if not ("_test.json" in f) and not ("Pre-measureStockSolution" in f):
            remove(f)

    # get the target data
    with open(conf['test_densiViscoDriver']['rawFile']) as file_raw:
        raw_target = file_raw.read()
    with open(conf['test_densiViscoDriver']['resultFile']) as file_result:
        result_target = file_result.read()

    # output file name
    outFileName = conf['test_densiViscoDriver']['outputFile'].split('\\')[-1].strip('.csv')

    # get the result from the function
    densiVisco_driver.retrieveData(sampleName=outFileName.split('_')[0], method=outFileName.split('_')[1], methodtype=outFileName.split('_')[2], savePath=conf['test_densiViscoDriver']['savePath'], outputPath=conf['densiViscoDriver']['outputFolder'])

    # load and read the files generated by the function
    with open(f'{conf["test_densiViscoDriver"]["savePath"]}\\{outFileName.split("_")[0]}_raw.json') as file_raw:
        raw_result = file_raw.read()
    with open(f'{conf["test_densiViscoDriver"]["savePath"]}\\{outFileName.split("_")[0]}_result.json') as file_result:
